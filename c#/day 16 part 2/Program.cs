...
int computeResult(int x, int y, int direction)
{
    int result = 0;
    energized = new bool[size, size];
    visitedFrom = new bool[size, size, 4];
...
{
    int result = computeResult(i, -1, S);
    if(result > maxResult)
        maxResult = result;
    result = computeResult(i, size, N);
    if(result > maxResult)
        maxResult = result;
    result = computeResult(-1, i, E);
    if(result > maxResult)
        maxResult = result;
    result = computeResult(size, i, W);
    if(result > maxResult)
        maxResult = result;
}
